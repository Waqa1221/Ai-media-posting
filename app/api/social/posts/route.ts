  ...
  try {
    ...
    if (!social_account_id || !platform || !content) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: social_account_id, platform, content",
        },
        { status: 400 }
      );
    }
    ...
    const platformRequirements =
      PlatformClientFactory.getPlatformRequirements(platform);
    if (
      platformRequirements &&
      content.length > platformRequirements.maxTextLength
    ) {
      return NextResponse.json(
        {
    ...
    if (
      platformRequirements?.requiresMedia &&
